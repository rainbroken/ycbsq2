...
* @filename:  hk32f0xxa_rtc.c
* @brief:     This file provides firmware functions to rtc
* @author:    AE Team
...
/** @defgroup RTC_Private_Defines RTC_Private_Defines
  * @{
  */
...
/**
  ...
  * @param  RTC_AlarmStruct: pointer to a @arg RTC_AlarmTypeDef structure which
  *         will be initialized.
  * @retval None
  */
...
void RTC_GetAlarm(uint32_t RTC_Format, uint32_t RTC_Alarm, RTC_AlarmTypeDef* RTC_AlarmStruct)
{
    ...
    else
    {
        if (RTC_Alarm == RTC_Alarm_B)
        {
            ...
            /* Fill the structure with the read parameters */
            RTC_AlarmStruct->RTC_AlarmTime.RTC_Hours = (uint32_t)((tmpreg & (RTC_ALRMBR_HT | \
                    RTC_ALRMBR_HU)) >> 16);
            RTC_AlarmStruct->RTC_AlarmTime.RTC_Minutes = (uint32_t)((tmpreg & (RTC_ALRMBR_MNT | \
                    RTC_ALRMBR_MNU)) >> 8);
            RTC_AlarmStruct->RTC_AlarmTime.RTC_Seconds = (uint32_t)(tmpreg & (RTC_ALRMBR_ST | \
                    RTC_ALRMBR_SU));
            RTC_AlarmStruct->RTC_AlarmTime.RTC_H12 = (uint32_t)((tmpreg & RTC_ALRMBR_PM) >> 16);
            RTC_AlarmStruct->RTC_AlarmDateWeekDay = (uint32_t)((tmpreg & (RTC_ALRMBR_DT | RTC_ALRMBR_DU)) >> 24);
            RTC_AlarmStruct->RTC_AlarmDateWeekDaySel = (uint32_t)(tmpreg & RTC_ALRMBR_WDSEL);
            RTC_AlarmStruct->RTC_AlarmMask = (uint32_t)(tmpreg & RTC_AlarmMask_All);
        }
    }
...
void RTC_AlarmSubSecondConfig(uint32_t RTC_Alarm, uint32_t RTC_AlarmSubSecondValue, uint8_t RTC_AlarmSubSecondMask)
{
    ...
    /* Configure the Alarm SubSecond register */
    if (RTC_Alarm == RTC_Alarm_A)
    {
        RTC->ALRMASSR = tmpreg;
    }
...
/**
  ...
  * @param  RTC_DayLightSaving: the value of hour adjuHKent.
  *         This parameter can be one of the following values:
...
void RTC_TamperPinsPrechargeDuration(uint32_t RTC_TamperPrechargeDuration)
{
    ...
    RTC->TAFCR &= (uint32_t)~(RTC_TAFCR_TAMP_PRCH);
    /* Configure the RTC_TAFCR register */
...
void RTC_TamperPullUpCmd(FunctionalState NewState)
{
    ...
    if (NewState != DISABLE)
    {
        ...
        RTC->TAFCR &= (uint32_t)~RTC_TAFCR_TAMP_PUDIS;
    }
    else
    {
...
/**
  * @brief  Convert from two digit BCD to Binary.
