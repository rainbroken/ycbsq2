...
* @filename:  hk32f0xxa_eval_i2c_ee.c
* @brief:     This file provides a set of functions needed to manage an I2C M24LR64
*             EEPROM memory.
*             ===================================================================
*             Notes:
*             - This driver is intended for HK32F0xxA families devices only.
...
*             from/to this memory. The needed HK32F0xxA hardware resources (I2C and
*             GPIO) are defined in hk32f0xxa_eval.h file, and the initialization is
*             performed in sEE_LowLevel_Init() function declared in hk32f0xxa_eval.c
*             file.
*             You can easily tailor this driver to any other development board,
*             by just adapting the defines for hardware resources and
*             sEE_LowLevel_Init() function.
*             @note In this driver, basic read and write functions (sEE_ReadBuffer() and
*             sEE_WritePage()) use Polling mode to perform the data transfer
*             to/from EEPROM memory.
*             +-----------------------------------------------------------------+
*             |                        Pin assignment                           |
*             +---------------------------------------+-----------+-------------+
*             |  HK32F0xxA I2C Pins                   |   sEE     |   Pin       |
*             +---------------------------------------+-----------+-------------+
*             | .                                     |   E0(GND) |    1  (0V)  |
*             | .                                     |   AC0     |    2        |
*             | .                                     |   AC1     |    3        |
...
*             +---------------------------------------+-----------+-------------+
* @author:    AE Team
* @version:   V1.0.1/2024-04-10
*             1.In I2C_SoftEnd_Mode,At the end of data transmission,
*               wait for TXDR to be empty before generating a stop signal
...
/** @addtogroup HK32F0xxA_EVAL_I2C_EE
  * @brief      This file includes the I2C EEPROM driver of HK32F0xxA-EVAL board.
  * @{
  */
...
/** @defgroup HK32F0xxA_EVAL_I2C_EE_Private_Functions
  * @{
  */
...
uint32_t sEE_WritePage(uint8_t* pBuffer, uint16_t WriteAddr, uint8_t* NumByteToWrite)
{
    ...
    I2C_TransferHandling(sEE_I2C, sEEAddress, (uint8_t)(*NumByteToWrite), I2C_SoftEnd_Mode, I2C_No_StartStop);
    ...
    /* Add wait until TXDR empty befor STOP */
    sEETimeout = sEE_LONG_TIMEOUT;
...
uint32_t sEE_WaitEepromStandbyState(void)
{
    ...
    in hk32f0xxa_eval_i2c_ee.h file) */
