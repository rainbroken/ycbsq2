...
/**
  ...
  * 说    明：调用此函数后，要想真正地呈现在屏幕上，还需调用更新函数
  */
void OLED_Spin(int8_t X, int8_t Y, int8_t Width, int16_t Angle)//旋转
{
	...
	for (y = Y - Width; y < Y + Width; y ++)		//遍历指定页
	{
		for (x = X - Width; x < X + Width; x ++)	//遍历指定列
		{
			...
			{
				OLED_DrawPoint(
				(x - X) * cos(Theta) - (y - Y) * sin(Theta) + X + 0.5,
				(x - X) * sin(Theta) + (y - Y) * cos(Theta) + Y + 0.5
				);
...
/*****************jiangxiekeji.com*****************/
typedef uint8_t u8;
void blend_pic(u8 Y,u8 width,u8 height,const u8* Img1,const u8* Img2,u8* blend_buf)
{
    int16_t i = 0,j = 0;
    ...
    {
        tmp1 = 0, tmp2 = 0;
        //将每一列的数据融合进uint64_t数据里
        for(j = 0; j < height/8; j++)
        {
            tmp1 |= (tmp1<<8)&Img1[i+j*width];
            tmp2 |= (tmp2<<8)&Img2[i+j*width];
        }
        tmp1<<=Y;
        tmp2>>=(height - Y);
        ...
        for(j = (height/8-1); j>=0; j--)
        {
            blend_buf[i+j*width] = (tmp1 & 0x00000000000000FF);
            tmp1>>=8;
        }
    }
}
