...
/** @defgroup EXTI_Init_Functions EXTI_Init_Functions
  * @{
  */
...
/**
  * @brief  Initializes the EXTI2 peripheral according to the specified
  *         parameters in the EXTI_InitStruct.
  * @param  EXTI_InitStruct: pointer to a EXTI_InitTypeDef structure that
  *         contains the configuration information for the EXTI peripheral.
  * @retval None
...
void EXTI2_Init(EXTI_InitTypeDef* EXTI_InitStruct)
{
    ...
    assert_param(IS_EXTI_LINE(EXTI_InitStruct->EXTI_Line));
    assert_param(IS_FUNCTIONAL_STATE(EXTI_InitStruct->EXTI_LineCmd));
    tmp = (uint32_t)EXTI2_BASE;

    if (EXTI_InitStruct->EXTI_LineCmd != DISABLE)
    {
        /* Clear EXTI line configuration */
        EXTI->IMR2 &= ~EXTI_InitStruct->EXTI_Line;
        EXTI->EMR2 &= ~EXTI_InitStruct->EXTI_Line;
        tmp += EXTI_InitStruct->EXTI_Mode;
        *(__IO uint32_t *) tmp |= EXTI_InitStruct->EXTI_Line;
        /* Clear Rising Falling edge configuration */
        EXTI->RTSR2 &= ~EXTI_InitStruct->EXTI_Line;
        EXTI->FTSR2 &= ~EXTI_InitStruct->EXTI_Line;
...
FlagStatus EXTI2_GetFlagStatus(uint32_t EXTI_Line)
{
    ...
    if ((EXTI->PR2 & EXTI_Line) != (uint32_t)RESET)
    {
        bitstatus = SET;
    }
    else
