...
******************************例程详细描述*****************************
功能描述:
1. 此示例提供了一个小型应用程序，其中按钮用于使用DMA并通过使用I2S固件库来触发I2S通信；
...
4. 使用DMA1信道3发送I2S_Buffer_Tx，并且使用DMA1通道2将接收到的值加载在I2S_Buffer_Rx缓冲
器中。只有有效的16个MSB被发送和接收，而剩下的32个数据包的16个LSB被0值填充，并且不产生任何中断。
