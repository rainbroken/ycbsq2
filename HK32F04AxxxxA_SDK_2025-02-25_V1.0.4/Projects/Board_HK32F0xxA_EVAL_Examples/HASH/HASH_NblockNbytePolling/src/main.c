...
/** @addtogroup HASH_NblockNbytePolling
  * @{
  */
...
#else
    __ALIGN_BEGIN const uint8_t aInput[] __ALIGN_END = "abc";//3
    /* The correct HASH information */
...
static void Display_MainMessage(void)
{
    ...
    printf("\n\r ======================================\n\r");
    printf(" ====  HASH Using polling Example  ====\n\r");
    printf(" ======================================\n\r");
    printf(" ---------------------------------------\n\r");
...
/**
  ...
  * @retval None
  */
ErrorStatus HASH_SHA256(void)
{
    __IO uint16_t nbvalidbitsdata = 0;
    __IO uint32_t counter = 0;
    uint32_t busystatus = 0;
    uint32_t inputaddr  = (uint32_t)aInput;
    ErrorStatus status = SUCCESS;

    /* Number of valid bits in last word of the Input data */
    nbvalidbitsdata = 8 * (HASH_INPUTSEIZE % 4);

    /* Configure the number of valid bits in last word of the message */
    HASH_SetLastWordValidBitsNbr(nbvalidbitsdata);

    /* Write input buffer in Data register */
    for (uint32_t index = 0 ; index < HASH_INPUTSEIZE; index += 4)
    {
    ...
    do
    {
        ...
        counter++;
    } while ((counter != HASH_TIMEOUT) && (busystatus != RESET));
...
void HASH_Function_Handle(void)
{
    ...
    {
        printf("HASH Using polling  is failed\r\n");
    }
}
