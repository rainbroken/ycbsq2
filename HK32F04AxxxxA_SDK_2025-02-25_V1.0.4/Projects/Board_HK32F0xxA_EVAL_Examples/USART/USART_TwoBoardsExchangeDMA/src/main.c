...
void USARTx_DMA_Send(uint8_t *sendbuf, uint16_t sendlen);
void USARTx_DMA_Recv(uint8_t *recvbuf, uint16_t recvlen);
...
int main(void)
{
    ...
    /* To use USART2 DMA, remapping needs to be enabled */
    if (USARTx == USART2)
    {
    ...
    while (1)
    {
        ...
        /* DMA channel Rx of USART Configuration and ready to recv */
        USARTx_DMA_Recv(CmdBuffer, 2);
        ...
        if ((PressedButton != JOY_NONE) && (CmdBuffer[0x00] == 0x00))
        {
            ...
            /* DMA channel Tx of USART Configuration and send */
            USARTx_DMA_Send(CmdBuffer, 2);
            ...
            /* Wait the USART DMA Rx transfer complete or time out,recv Ack buffer */
            TimeOut = USER_TIMEOUT;
            ...
            /* DMA channel Tx of USART Configuration and send*/
            USARTx_DMA_Send(TxBuffer, CmdBuffer[0x01]);
        ...
        if (CmdBuffer[0x00] != 0x00)
        {
            ...
            /* The transmitter After receive the ACK command it sends the defined data
               in his TxBuffer */
            /* DMA channel Rx of USART Configuration */
            USARTx_DMA_Recv(RxBuffer, CmdBuffer[0x01]);
...
void USARTx_Config(uint32_t Baud)
{
    ...
    usart_struct.USART_Parity = USART_Parity_No;                                /* No parity */
    usart_struct.USART_StopBits = USART_StopBits_1;                             /* One Stop Bit */
