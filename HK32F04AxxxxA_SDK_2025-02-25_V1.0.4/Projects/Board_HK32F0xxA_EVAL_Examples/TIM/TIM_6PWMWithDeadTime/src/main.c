...
/** @addtogroup TIM_6PWMWithDeadTime
  * @{
  */
...
static void TIM_Config(void)
{
    ...
    GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_DOWN;  //default no break
    GPIO_Init(GPIOB, &GPIO_InitStructure);
    ...
    1/ Generate 3 complementary PWM signals with 3 different duty cycles:
      ...
      TIM1CLK =  PCLK2
      PCLK2 = 1/2 HCLK
      => TIM1CLK  = PCLK2*2 = HCLK = SystemCoreClock

      TIM1CLK is fixed to SystemCoreClock, the
      TIM1 Prescaler is equal to 0 so the
      TIM1 counter clock used is SystemCoreClock (56MHz).
