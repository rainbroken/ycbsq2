...
uint16_t SRC_Buffer[3] __attribute__ ((aligned (4))) = {0x0FFF, 0x0000, 0x0555};
...
static void TIM_Config(void)
{
    ...
    /* -----------------------------------------------------------------------
      ...
      TIM1 input clock (TIM1CLK) is set to APB2 clock (PCLK2)
        TIM1CLK = PCLK2
        PCLK2 = 1/2HCLK
        => TIM1CLK = 2*PCLK2 = HCLK = SystemCoreClock

      To get TIM1 counter clock at 28 MHz, the prescaler is computed as follows:
        Prescaler = (TIM1CLK / TIM1 counter clock) - 1
        Prescaler = (SystemCoreClock /28 MHz) - 1

     TIM1 Frequency = TIM1 counter clock/(ARR + 1)
                                                 = 28 MHz / 4096 = 6.83 KHz
    ...
    TIM_TimeBaseStructure.TIM_Prescaler = (uint16_t) (SystemCoreClock / 28000000) - 1;
