...
******************************例程详细描述*****************************
功能描述:
1. 此示例提供了一个小型应用程序，使用SPI中断的方式处理SPI通信数据；
...
4. 主设备向从设备发送特定命令，并等待来自从设备的ACK命令；
5. 主设备向从设备发送特定命令，该命令包含事务代码
（CMD_RIGHT、CMD_LEFT、CMD_UP、CMD_DOWN或CMD_SEL），并从设备接收ACK命令；
6. 从设备接收命令并发送ACK命令；
7. 主设备将定义的NumberOfByte从TxBuffer发送到从设备，并从从设备接收到Rxbuffer；
8. 从设备将定义的NumberOfByte从Txbuffer发送到主设备，并从主设备接收到Rxbuffer
中的NumberOfByte；
9. 主控器将接收到的数据与TxBuffer进行比较，并检查ACK命令。LED亮起表示接收到的正确数据，如下所示：
