...
void SPI1_IRQHandler(void)
{
    ...
    if (SPI_I2SGetITStatus(SPIx, SPI_I2S_IT_TXE) == SET)
    {
        ...
        if (Tx_Idx == GetVar_NbrOfData())
        {
            ...
            SPI_I2SITConfig(SPIx, SPI_I2S_IT_TXE, DISABLE);
        }
    }

    /* SPI in Slave Receiver mode--------------------------------------- */
    if (SPI_I2SGetITStatus(SPIx, SPI_I2S_IT_RXNE) == SET)
    {
        if (CmdReceived == 0x00)
        {
    ...
    /* SPI Error interrupt--------------------------------------- */
    if (SPI_I2SGetITStatus(SPIx, SPI_I2S_IT_OVR) == SET)
    {
        SPI_ReceiveData8(SPIx);
        SPI_I2SGetITStatus(SPIx, SPI_I2S_IT_OVR);
    }
